...
export default registerAs('search', () => {
  ...
  const indexes = {
    // 用户索引
    user: {
      enabled: process.env.SEARCH_INDEX_USER_ENABLED !== 'false',
      alias: 'users',
      indexPattern: 'users-*',
      settings: {
        number_of_shards: process.env.ES_INDEX_USER_SHARDS
          ? parseInt(process.env.ES_INDEX_USER_SHARDS, 10)
          : 5,
        number_of_replicas: process.env.ES_INDEX_USER_REPLICAS
          ? parseInt(process.env.ES_INDEX_USER_REPLICAS, 10)
          : 2,
        refresh_interval: process.env.ES_INDEX_USER_REFRESH_INTERVAL || '30s',
        // 分析器配置
        analysis: {
          analyzer: {
            default: {
      ...
      // 搜索配置
      search: {
        fields: process.env.SEARCH_INDEX_USER_FIELDS
          ? process.env.SEARCH_INDEX_USER_FIELDS.split(',').map((field) =>
              field.trim(),
            )
          : ['user_name', 'name', 'email', 'phone', 'department'],
        weights: {
          user_name: process.env.SEARCH_INDEX_USER_WEIGHT_USERNAME
            ? parseInt(process.env.SEARCH_INDEX_USER_WEIGHT_USERNAME, 10)
            : 5,
          name: process.env.SEARCH_INDEX_USER_WEIGHT_NAME
            ? parseInt(process.env.SEARCH_INDEX_USER_WEIGHT_NAME, 10)
            : 4,
          email: process.env.SEARCH_INDEX_USER_WEIGHT_EMAIL
            ? parseInt(process.env.SEARCH_INDEX_USER_WEIGHT_EMAIL, 10)
            : 3,
          phone: process.env.SEARCH_INDEX_USER_WEIGHT_PHONE
            ? parseInt(process.env.SEARCH_INDEX_USER_WEIGHT_PHONE, 10)
            : 3,
          department: process.env.SEARCH_INDEX_USER_WEIGHT_DEPARTMENT
            ? parseInt(process.env.SEARCH_INDEX_USER_WEIGHT_DEPARTMENT, 10)
            : 2,
        },
      },
    },
    // 组织索引
    organization: {
      enabled: process.env.SEARCH_INDEX_ORGANIZATION_ENABLED !== 'false',
      alias: 'organizations',
      indexPattern: 'organizations-*',
      settings: {
        number_of_shards: process.env.ES_INDEX_ORG_SHARDS
          ? parseInt(process.env.ES_INDEX_ORG_SHARDS, 10)
          : 5,
        number_of_replicas: process.env.ES_INDEX_ORG_REPLICAS
          ? parseInt(process.env.ES_INDEX_ORG_REPLICAS, 10)
          : 2,
        refresh_interval: process.env.ES_INDEX_ORG_REFRESH_INTERVAL || '30s',
        analysis: {
          analyzer: {
            default: {
              type: 'standard',
      ...
      search: {
        fields: process.env.SEARCH_INDEX_ORGANIZATION_FIELDS
          ? process.env.SEARCH_INDEX_ORGANIZATION_FIELDS.split(',').map(
              (field) => field.trim(),
            )
          : ['name', 'description', 'address', 'contact'],
        weights: {
          name: process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_NAME
            ? parseInt(process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_NAME, 10)
            : 5,
          description: process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_DESCRIPTION
            ? parseInt(
                process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_DESCRIPTION,
                10,
              )
            : 3,
          address: process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_ADDRESS
            ? parseInt(process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_ADDRESS, 10)
            : 2,
          contact: process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_CONTACT
            ? parseInt(process.env.SEARCH_INDEX_ORGANIZATION_WEIGHT_CONTACT, 10)
            : 3,
        },
      },
    },
